...
| 数据 | 来源 | 说明 |
|---|---|---|
| 作者信息 | `users/Yutong-Fan` API | 名字 / bio 实时获取，hero、页脚、页面标题自动填充 |
...
**部署**：Cloudflare Pages 新建项目 → 连接 GitHub 仓库 `Yutong-Fan/openweb-wiki` → 构建设置保持默认（纯静态，无构建命令）→ 绑定域名 `openweb.wiki`。之后每次 push 到 main 自动构建上线。
...
- **条目地址直达**：`https://openweb.wiki/#/entries/OW-001` 直接打开对应条目，浏览器前进 / 后退联动
- **一键复制地址**：每条目可复制完整可访问 URI（`https://openweb.wiki/#/entries/xxx`）
...
- **浅色**：底 `#F6F7F9` / 卡 `#FFFFFF` / 文字 `#101828` / 主色 `#0E9384`
- **深色**：底 `#121212`（纯中性）/ 卡 `#1E1E1E` / 文字 `#E8E8E8`（对比 15.3:1）/ 主色压灰 teal `#7FB5AE`（8.1:1）—— 同色相暗化，无荧光、不刺眼
- **字体**：系统字体栈（中文渲染最干净），等宽仅用于 ID / 日期 / star 元数据
- **布局**：sticky 毛玻璃 header → hero 一行 → 分类 pill → auto-fill 卡片网格（300px 起，1 → 2 → 3 列平滑过渡）
- **卡片**：12px 圆角、轻阴影、hover 上浮 3px
- **动画**：卡片入场 stagger + hover 提升 + 主题切换 0.3s 颜色过渡，克制不堆砌
- **图标**：Lucide SVG sprite（禁 emoji），em 单位随字号缩放
- **无障碍**：focus-visible 焦点环、skip-link、aria-live、触摸目标 ≥40px、`prefers-reduced-motion` 归零、对比度 AA
